...
def get_sec_filings() -> dict:
    """
    Retrieve Prologis SEC filing data (10-K, 10-Q) from AWS S3.
    Returns financial metrics including revenue, net income, and operating expenses.
    """
    try:
...
def get_press_releases(limit: int = 3) -> list:
    """
    Retrieve recent Prologis press releases from AWS S3.
    Args:
        limit: number of press releases to return (default 3)
    Returns list of press releases with title, date, and summary.
    """
    try:
        s3  = boto3.client("s3", region_name=AWS_REGION)
...
def ask_agent(user_message: str) -> str:
    ...
    try:
        ...
        try:
            ...
            # Build context manually for fallback
            sec = get_sec_filings()
            prs = get_press_releases(2)
            ctx = ""
            if sec and "error" not in sec:
                ctx += f"SEC Data: {json.dumps(sec)[:600]}. "
            if prs:
                ctx += f"Press Releases: {json.dumps(prs)[:300]}. "
